...
char* update_page(SOCKET ClientSocket,char *htm_temp_path,char* htm_path)
{
    ...
    char option_str[PATTERN_LEN];
    char option_str__[PATTERN_LEN];
    int form_index=0;
    int cur_select;
    int len=0;
    ...
    htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__,  TIMER_SETUP, TIMER_SETUP);
    htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__,  TIMER_READ , TIMER_READ );

    if(RTC_flag == YES)
        sprintf(option_str,"当前时%d:%d",(int)*pRTC_hour, (int)*pRTC_minute );
    else
        sprintf(option_str,"已运行%d:%d",(int)*pRTC_hour, (int)*pRTC_minute );
    htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__, "_time" , option_str );

    for(i=1;i<3;i++){   //update select_power_type
        sprintf(option_str,"option_t%d",i);
        htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__,  option_str, select_power_type[i-1] );
    }
    for(i=1;i<5;i++){   //update select_time_unit
        sprintf(option_str,"option_u%d",i);
        htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__,  option_str, select_time_unit[i-1] );
    }

    sprintf(option_str,"%d",pStatus_timer->count1); // update remain_cycles
    htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__,  "remain_cycles", option_str );

    sprintf(option_str,"%d",pConfig_timer->count); // update cycles
    htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__,  "cycles", option_str );

    sprintf_RTC(option_str,pConfig_timer->down_time,RTC_flag );
    htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__,  "dtime0", option_str );
    //	printf("::: %s ; %s\n", form_value[form_index].dest,form_value[form_index].src);

    sprintf(option_str,"%d",pConfig_timer->up_time );
    htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__,  "utime0", option_str );


    for(i=1;i<9;i++){   //update group settings

        sprintf(option_str,"dtime%d",i);
        sprintf_RTC(option_str__, pGroup_timer->down_time, RTC_flag );
        htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__,  option_str, option_str__ );
        //		printf("===>%s ; %s\n", form_value[form_index].dest,form_value[form_index].src);

        sprintf(option_str,"utime%d",i);
        sprintf(option_str__, "%d", pGroup_timer->up_time );
        htm_replace(htm_buffer, PRE_CHAR, PRE_CHAR__,  option_str, option_str__ );
        //		printf("--->%s ; %s\n", form_value[form_index].dest,form_value[form_index].src);

        pGroup_timer++;
    }
    /*now all required replacement have been done */

...
extern int usbhidOpenDevice(usbDevice_t **device, int vendorID, char *vendorName, int productID, char *productName, int usesReportIDs);
 ...
 */
extern void    usbhidCloseDevice(usbDevice_t *device);
/* Every device opened with usbhidOpenDevice() must be closed with this function.
 */
extern int usbhidSetReport(usbDevice_t *device, char *buffer, int len);
 ...
 */
extern int usbhidGetReport(usbDevice_t *device, int reportID, char *buffer, int *len);
